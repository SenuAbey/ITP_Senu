...
function CustomerVehicleDetails() {
    const ComponentsRef = useRef(null);
      ...
      useEffect(()=>{
        fetchHandler().then((data) => {
          ...
          const availableVehicles = data.filter(vehicle => vehicle.vehicleStatus === "Available");
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle:'Vehicles Details Report' , 
        onAfterPrint:()=>{
    ...
    const handleFetchNewVehicles = async () => {
      try {
        const response = await axios.get("http://localhost:5000/vehicles?new=true"); // Fetch new vehicles
        setVehicles(response.data.Vehicles); // Update state with fetched vehicles
      } catch (err) {
        console.error("Error fetching new vehicles:", err); // Handle error if the fetch fails
      }
    };
  ...
  return (
<div>
      <h1>Customer Vehicle Details Display Page</h1>
      <br/>
      ...
      <button className="show-new-vehicles-btn" onClick={handleFetchNewVehicles}>
        Show New Vehicles
      </button>
      ...
      <div className="vehicle-details-container" ref ={ComponentsRef}>
        {Array.isArray(vehicles) && vehicles.length > 0 ? (
          vehicles.map((vehicle, i) => (
            <div key={i}>
              <CustomerVehicles vehicle={vehicle} />
